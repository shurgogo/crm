layui.use(['table','layer','treetable'],function() {
    ...
    table.on('tool(modules)', function(obj){
        ...
        switch(obj.event){
            ...
            case 'add':
                ...
                var grade=obj.data.grade + 1;
                if ( grade== 1) {
                    title = '资源管理 - 添加菜单';
                }else if (grade == 2) {
                    title = '资源管理 - 添加按钮';
                }
                openModuleDialog(title, 'module/toAddModule?grade=' + grade + '&parentId=' + obj.data.id);
                break;
            // 修改菜单或按钮
            case 'update':
                // 开启窗口
    ...
    table.on('toolbar(modules)', function(obj){
        ...
        switch(obj.event){
            // 全部展开事件
            case 'expand':
                treeTable.expandAll("#modules");
                break;
            // 全部折叠事件
            case 'fold':
                treeTable.foldAll("#modules");
                break;
            case 'add':
                // 添加
