layui.use(['table','layer','form'],function() {
    ...
    function formatterBtn(customerServe) {
        ...
        if (customerServe.state == 'fw_001') {
            ...
            // 删除、修改按钮可用
            return "<a class='layui-btn layui-btn-xs layui-btn-warm' lay-event='pass'>审核</a>" +
    ...
    table.on('toolbar(customerServe)', function(obj){
        ...
        switch(obj.event){
            ...
            case 'add':
                // 开启子窗口
                openCustomerServeDialog("服务管理 - 服务创建", "customer_serve/toAddAndUpdatePage");
                break;
        };
    });
    ...
    table.on('tool(customerServe)', function(obj){
        switch(obj.event){
            case 'edit':
                // 修改
                openCustomerServeDialog("服务管理 - 服务创建修改", "customer_serve/toAddAndUpdatePage?id=" + obj.data.id);
                break;
            // 删除
            case 'del':
